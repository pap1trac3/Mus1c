...
class VaultRepository {
  ...
  /**
   * Nearest learned profiles to a vector, for duplicate detection.
   *
   * Scoped to profiles because the question it answers is "does the vault
   * already know this style", which an ingested lyric chunk cannot answer.
   */
  async findSimilarProfiles(vector, { limit = 5, includeSimilarity = true } = {}) {
    const cursor = this.collection.find(
  ...
  /**
   * Specific profiles by document id, for a caller that named them rather
   * than searching. Kind-scoped like every other profile read, and the vector
   * is projected out: the caller wants the blueprint, not 1536 floats.
   */
  async findProfilesByIds(documentIds) {
    if (!Array.isArray(documentIds) || documentIds.length === 0) return [];

    const cursor = this.collection.find(
      { 'metadata.kind': STYLE_PROFILE_KIND, 'metadata.document_id': { $in: documentIds } },
      { limit: documentIds.length, projection: { $vector: 0 } }
    );
    return cursor.toArray();
  }
