...
// Word-shaped tokens, kept with their column so a marker row can be printed
// underneath the lyric exactly as it reads.
const WORD_TOKEN = /[a-z']+/gi;

/**
 * Where the beats fall inside a line, at the same even eighth-note delivery the
 * bar grid assumes.
 *
 * Returns one entry per beat the line covers, naming the word that beat lands
 * on or inside, the column that word starts at, and whether the beat coincides
 * with the start of the word. A sheet can print a marker row underneath the
 * lyric from this, and say truthfully which beats land between words rather
 * than nudging them onto the nearest one.
 *
 * This is rhythmic placement, not lexical stress: nothing here knows which
 * syllable of "forever" carries the accent, and claiming to without a
 * pronunciation dictionary would be guesswork. What it does say is true and
 * useful before tracking — "at this density, this is the word that hits the
 * one".
 */
function beatPositions(line, { syllablesPerBar = SYLLABLES_PER_BAR, beatsPerBar = BEATS_PER_BAR } = {}) {
  // Blank the bracketed tags rather than delete them, so every column still
  // lines up with the lyric as it will be printed.
  const scannable = String(line || '').replace(BRACKETED, (tag) => ' '.repeat(tag.length));

  const words = [];
  let syllable = 0;
  for (const match of scannable.matchAll(WORD_TOKEN)) {
    const word = match[0];
    if (!/[a-z]/i.test(word)) continue;
    words.push({ word, column: match.index, start: syllable });
    syllable += countSyllables(word);
  }

  const totalSyllables = syllable;
  const syllablesPerBeat = syllablesPerBar / beatsPerBar;
  const beats = [];

  for (let index = 0; index * syllablesPerBeat < totalSyllables; index++) {
    const at = index * syllablesPerBeat;
    // The last word whose syllables have already begun by this point.
    let carrier = words[0];
    for (const word of words) {
      if (word.start <= at) carrier = word;
      else break;
    }
    if (!carrier) break;
    ...
    beats.push({
      ...
      on_word_start: carrier.start === at,
    });
  }
...
function buildBarGrid(text, { bpm, syllablesPerBar = SYLLABLES_PER_BAR } = {}) {
  ...
  const rows = lines.map((line) => {
    ...
    return {
      ...
      // Carried on the row so an exported sheet can mark the beats without
      // re-counting syllables in the browser, where the counter does not live.
      beats: beatPositions(line, { syllablesPerBar, beatsPerBar: BEATS_PER_BAR }),
    };
  });
